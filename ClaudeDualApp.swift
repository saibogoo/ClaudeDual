...
final class AppUpdateManager: ObservableObject {
    ...
    func downloadAvailableUpdate() async {
        ...
        do {
            ...
            request.setValue("ClaudeDual/\(currentVersion)", forHTTPHeaderField: "User-Agent")
            ...
            statusMessage = "安装包已校验并打开，请将 ClaudeDual 拖入“应用程序”完成升级。"
    ...
    private func loadGitHubAPIRelease() async throws -> AppUpdateRelease {
        ...
        return AppUpdateRelease(
            ...
            title: responseRelease.name ?? "ClaudeDual \(responseRelease.tagName)",
...
class ClaudeDualManager: ObservableObject {
    ...
    private func saveCodexConfigToDisk() -> Bool {
        ...
        var lines = [
            "# Generated by ClaudeDual. This file belongs only to the isolated Codex instance.",
            "model = \"\(tomlString(profile.effectiveUpstreamModel))\"",
            "model_provider = \"\(providerId)\"",
            "cli_auth_credentials_store = \"file\"",
            "",
            "[model_providers.\(providerId)]",
            "name = \"ClaudeDual \(tomlString(profile.name))\"",
...
struct ContentView: View {
    ...
    var body: some View {
        ...
        .alert("未检测到 \(manager.selectedClient.desktopTitle)", isPresented: $showClientInstallPrompt) {
            ...
            Text("ClaudeDual 需要 \(manager.selectedClient.desktopTitle) 才能启动隔离实例。请先安装客户端，然后返回并重新检测。")
        }
